...
ALTER TABLE usuarios ADD CONSTRAINT fk_usuario_prof FOREIGN KEY (profissional_id) REFERENCES profissionais(id) ON DELETE CASCADE;
...
CREATE TABLE agendamentos (id INT AUTO_INCREMENT PRIMARY KEY,nome VARCHAR(100) NOT NULL,telefone VARCHAR(20) NOT NULL,nascimento DATE NOT NULL,servico VARCHAR(100) NOT NULL,profissional_id INT NOT NULL,data DATE NOT NULL,horario TIME NOT NULL,status VARCHAR(20) NOT NULL DEFAULT 'agendado',token VARCHAR(64) NOT NULL UNIQUE,criado_por INT NULL,criado_em TIMESTAMP DEFAULT CURRENT_TIMESTAMP,CONSTRAINT fk_agendamento_prof FOREIGN KEY(profissional_id) REFERENCES profissionais(id) ON DELETE CASCADE,CONSTRAINT fk_agendamento_criador FOREIGN KEY(criado_por) REFERENCES usuarios(id) ON DELETE CASCADE,INDEX idx_prof_data(profissional_id,data),INDEX idx_status(status));
